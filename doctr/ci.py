"""
Functions that are common to all CIs
"""
...
class CI:
    ...
    def setup_deploy_key(self, keypath='github_deploy_key', key_ext='.enc', env_name='DOCTR_DEPLOY_ENCRYPTION_KEY'):
        """
        ...
        self.decrypt_file(keypath + key_ext, key)
    ...
    def setup_GitHub_push(self, deploy_repo, *, auth_type='deploy_key',
        ...
        """
        Setup the remote to push to GitHub (to be run on CI).
        ...
        r = requests.get(REPO_URL.format(slug=self.repo_slug()))
        fork = r.json().get('fork', False)

        canpush = self.determine_push_rights(
            branch_whitelist=branch_whitelist,
            branch=self.branch(),
            pull_request=self.is_pull_request(),
            fork=fork,
            tag=tag,
            build_tags=build_tags)

        print("Setting git attributes")
        self.set_git_user_email()
        ...
        if canpush:
            if auth_type == 'token':
                ...
                key_ext = '.' + key_ext
                try:
                    self.setup_deploy_key(keypath=keypath, key_ext=key_ext, env_name=env_name)
                except RuntimeError:
    ...
    def checkout_deploy_branch(self, deploy_branch, canpush=True):
        """
        ...
        self.create_deploy_branch(deploy_branch, push=canpush)
    ...
    def deploy_branch_exists(self, deploy_branch):
        """
    ...
    def create_deploy_branch(self, deploy_branch, push=True):
        """
    ...
    def commit_docs(self, *, added, removed):
        """
    ...
    def push_docs(self, deploy_branch='gh-pages', retries=5):
        """
    ...
    def determine_push_rights(self, *, branch_whitelist, branch,
        pull_request, tag, build_tags, fork):
        """Check if CI is running on ``master`` (or a whitelisted branch) to
        ...
        if not any([re.compile(x).match(branch) for x in branch_whitelist]):
            ...
            print("This is the {branch} branch".format(branch=branch), file=sys.stderr)
            canpush = False
...
def get_token():
    """
    Get the encrypted GitHub token in the CI.
